...
export default function App() {
  ...
  useEffect(() => {
    fetch('https://tree-hierarchy-app-backend.onrender.com/api/nodes/root')
      .then(r => r.json())
      .then(data => {
  ...
  useEffect(() => {
    if (search.length > 2) {
      fetch(`https://tree-hierarchy-app-backend.onrender.com/api/search?q=${search}`)
        .then(r => r.json())
  ...
  const toggleNode = async (id: string) => {
    if (expanded.has(id)) {
      ...
      if (!children.has(id)) {
        const res = await fetch(`https://tree-hierarchy-app-backend.onrender.com/api/nodes/${id}/children`);
